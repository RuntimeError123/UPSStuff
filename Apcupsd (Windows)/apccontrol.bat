...
:commfailure
   ...
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'Communications with UPS lost.'"
   GOTO :done

:commok
   %POPUP% "Communciations with UPS restored."
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'Communciations with UPS restored.'"
   GOTO :done
...
:failing
   ...
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'UPS battery power exhausted. Doing shutdown.'"
   GOTO :done

:timeout
   %POPUP% "UPS battery runtime limit exceeded. Doing shutdown."
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'UPS battery runtime limit exceeded. Doing shutdown.'"
   GOTO :done

:loadlimit
   %POPUP% "UPS battery discharge limit reached. Doing shutdown."
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'UPS battery discharge limit reached. Doing shutdown.'"
   GOTO :done

:runlimit
   %POPUP% "UPS battery runtime percent reached. Doing shutdown."
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'UPS battery runtime percent reached. Doing shutdown.'"
   GOTO :done
...
:changeme
   ...
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'Emergency! UPS batteries have failed: Change them NOW'"
   GOTO :done

:remotedown
   %POPUP% "Shutdown due to master state or comms lost."
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'Shutdown due to master state or comms lost.'"
   GOTO :done
...
:battdetach
   ...
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'Battery disconnected'"
   GOTO :done

:battattach
   %POPUP% "Battery reattached"
   powershell.exe -Command ".\Send-TelegramNotification.ps1 -Message 'Battery reattached'"
   GOTO :done
